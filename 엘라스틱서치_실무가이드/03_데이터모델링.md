...
- 타임아웃 설정
  ...
  - 기본 1분 대기, 초과시 요청 실패. 이때 timeout 파라미터를 설정해서 대기 시간 조정 가능
  - ex) 5분간 다른 색인 API가 완료되기를 기다리는 예.
...
- 인덱스 매핑 정보 자동 생성
  ...
  - 기본적으로는 동적 매핑을 허용하기 때문에 색인 즉시 새로운 필드가 새엉된다.
  - 동적 매핑을 서용할 경우 원하지 않는 오류가 발생할 수도 있기 때문에 상황에 따라서는 기능을 비활성화해야 한다.
  - 동적 매핑과 관련된 설정은 아래와 같다. (해당 옵션은 elaticsearch.yml에서 설정)

~~~
action.auto_create_index : 인덱스의 자동 생성 여부를 설정 default = true
index.mapper.dynamic : 동적 매핑 사용 여부 설정. default = true
~~~

### 3.5.2 Index API
- Index API는 문서를 특정 인덱스에 추가하는데 사용된다. 참고로 새로 추가된 문서는 버전 값으로 1이 부여되며, 문서가 업데이트될 때마다 버전 값이 1씩 증가한다.
- ex) 다음과 같이 문서 하나 추가
~~~
PUT movie_dynamic/_doc/1
{
...
~~~
- _shards 항목은 몇 개의 샤드에서 명령이 수행됐는지에 대한 정보를 나타내다. 여기서 total항목은 복제돼야 하는 전체 샤드 개수를 나타내며, successful은 성공적으로 복제된 샤드 개수를 나타낸다.
- failed는 복제에 실패한 샤드 건수를 나타낸다.
- Index API는 최소 한 개 이상의 successful 항목이 있어야 성공한 것으로 간주한다.

### 3.5.3 Get API
- Get API는 특정 문서를 인덱스에서 조회할 때 사용하는 API다. 조회하고자 하는 문서의 ID를 명시적으로 지정해서 사용
- 조회되는 문서의 내용은 _source 항목으로 확인
- 아래와 같이 인덱스 생성
...
Get API를 이용해 문서 조회 문서의 ID로 1을 지정
...
~~~
- 일반적인 상황에서는 모든 필드가 _source 필드의 일부분으로 저장된다. 필드의 데이터가 작을 때는 상관없진만 데이터가 한 권의 책 혹은 보고서처럼 매우 방대한 크기의 텍스트일 경우에는 무제가 될 수 있다.
- 이 경우 특정 필드를 _source 항목으로 제공되지 않도록 설정할 수 있는데, _source_exclude 옵션을 이용해 제외할 필드명을 지정할 수 있다.
...
### 3.5.4 Delete API
- Delete API를 이용하면 문서를 삭제할 수 있다. 그 결과, result 항목에 "deleted" 라는 값이 반환되며 _version값이 1만큼 증가한 것 확인 가능
~~~
...
- 특정 문서가 아니라 인덱스 전체를 삭제하고 싶을 때는 인덱스명을 입력하면 된다. 단, 인덱스가 삭제되면 포함된 모든 문서가 삭제되고 다시 복구 불가(주의 필요)
~~~
DELETE movie_dynamic
~~~

### 3.5.5 Delete By Query API
- 특정 인덱스에서 검색을 수행한 후 그 결과에 해당하는 문서만 삭제하고 싶을 경우 DeleteBy Query API를 사용하면 된다. 
- 다음과 같이 movieCd가 "20173732" 문서 생성
~~~
PUT movie_dynamic/_doc/1
{
  "movieCd":"20173732",
...
~~~
- Delete By Query API를 이용해 movieCd가 "20173732"인 문서를 검색한 후 검색된 문서를 삭제
~~~
...
~~~
- Delete By Query API를 호출하면 해당 인덱스의 스냅숏을 불러와 스냅숏이 있는 문서의 버전을 기반으로 삭제를 수행
- 대량의 수정작업이 진행중일 때 삭제를 수행하면 버전이 서로 일치하지 않게 되고 version_conflicts 항목을 통해 삭제에 실패한 문서의 건수 출력.

### 3.5.6 Update API
- Update API를 이용하면 스크립트를 바탕으로 문서 수정 가능.
- 스크립트를 토앻 "ctx._source.필드명"과 같은 형태로 접근 가능

~~~
PUT moive_dynamic/_doc/1
{
  "counter" : 1000,
...
- 엘라스틱서치에서 제공하는 Update는 Index에서 문서를 가져와 스크립트를 수행한 후, 이를 다시 재색인한다. 
- Update API를 사용하기 위해서는 _source 필드가 활성화돼 있어야 한다.
- ctx필드에서는 _source 변수뿐 아니라 _index, _type, _id, _version, _routing, _now등 추가적인 변수도 사용할 수 있다.

### 3.5.7 Bulk API
- Get/Delete/Update API는 한 번에 하나의 문서만들 대상으로 동작. 
- Bulk API를 이용하면 한 번의 호출로 다수의 문서를 색인하거나 삭제. 
- 색인 작업의 경우 한 번에 처리함으로써 색인 속도를 크게 향상.
- 대량 색인의 경우 bulk API를 사용하는 것이 좋다.
- ex)
...
- 여러번 호출을 한번에 하는 장점이 있지만 여러건의 데이터가 한번에 처리되기 때문에 도중에 실패가 발생하더라도 갱신되거나 수정된 결과는 롤백되지 않는다. 항상 처리 결과를 확인해야 한다.

### 3.5.8 Reindex API
- Reindex API는 한 인덱스에서 다른 인덱스로 문서 복사 시 사용.
- ex)
~~~
POST /_reindex
{
...
~~~
- source가 복사할 인덱스, dex가 복사될 인덱스 
- 특정 조회 결과와 일치하는 문서반 복사하고 싶을 경우 source 항목에 쿼리를 포함시켜 쿼리 결과에 일치하는 문서만 복사
~~~
POST /_reindex
{
  "source": {
...
~~~
- 검색 시 정렬 작업은 리소스를 많이 사용하기 떄문에 색인할 때 정렬된 상태로 색인할 수 있다면 좋을 것이다. 
- Reindex API를 이용해 특정 문서를 복사해서 새로운 인덱스를 만들 때 새로운 정렬 방식으로 데이터를 정렬한 후 복사도 가능.
- ex) 가장 많은 관객 수를 기준으로 정렬해서 movie_dynamic 인덱스에서 movie_dynamic_new 인덱스로 문서 복사
...
~~~
- Reindex API는 1,000건 단위로 스크롤을 수행. 증가를 원하면 size를 변경.